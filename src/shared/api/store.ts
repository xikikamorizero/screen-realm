...
class Movie {
  ...
  public readonly getBoxOffice = new ServiceStore<[types.ID], types.BoxOffice>(
    services.Movie.getBoxOffice,
    false
  );
...
export class Store {
  ...
  toggleBookmark(movie: types.Bookmarks) {
    const index = this.bookmarks.some(item => item.filmId === movie.filmId);
    if (index) {
      this.bookmarks = this.bookmarks.filter(item => item.filmId !== movie.filmId);
    } else {
